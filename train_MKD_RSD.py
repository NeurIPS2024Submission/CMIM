...
class CATModel(nn.Module):
    ...
    def forward(self, TeacherLogit, Target):
        TeacherProb = F.softmax(TeacherLogit/self.tempTrans, dim=1)
...
def trainOneEpoch(model, Teacher,RsdLayer, CatLayer, trainloader, CElossFn, KLlossFn, optimizer, CAToptimizer, params, alpha):
    model.train()
    Teacher.eval()
    Nsamples = 0
    Loss = 0
...
def main(params):
    ...
    for epoch in range(params.num_epochs):
        ...
        TrainRes = trainOneEpoch(model, Teacher,RsdLayer, CatLayer, trainloader, CElossFn, KLlossFn, optimizer, CAToptimizer, params, alpha)
