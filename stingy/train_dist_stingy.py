...
DistLoss = DIST_TeacherProb(beta=1.0, gamma=1.0, tau=1.0)

def distlossfn(outputs, labels, teacher_outputs):
    """
    Compute the knowledge-distillation (KD) loss given outputs, labels.
    """
    KD_loss = DistLoss(outputs, teacher_outputs) + nn.CrossEntropyLoss()(outputs, labels)
    return KD_loss


def trainOneEpoch(model, Teacher, StingyLayer, trainloader, optimizer):
    model.train()
    Teacher.eval()
    Nsamples = 0
    Loss = 0
    ...
    with tqdm(total=len(trainloader)) as t:
        for Nbatch, (train_batch, labels_batch) in enumerate(trainloader):
            ...
            loss = distlossfn(BatchLogits, labels_batch, TeacherLogits)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
def main(params):
    ...
    for epoch in range(params.num_epochs):
        TrainRes = trainOneEpoch(model, Teacher, StingyLayer, trainloader, optimizer)
