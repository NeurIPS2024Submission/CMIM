...
class KLLossfnWithT():
    ...
    def __call__(self,preds, teacher_Prob, target):
        teacher_Prob = teacher_Prob**(1/self.T)
        teacher_Prob = teacher_Prob/torch.sum(teacher_Prob,1)[:,None]
        UniTargets = target.unique()
        for UniTarget in UniTargets:
            Mask = (target == UniTarget)
            teacher_Prob[Mask] = teacher_Prob[Mask].mean(0)
...
def trainOneEpoch(model, Teacher,StingyLayer, trainloader, CElossFn, KLlossFn, optimizer, alpha):
    ...
    with tqdm(total=len(trainloader)) as t:
        for Nbatch, (train_batch, labels_batch) in enumerate(trainloader):
            ...
            softLoss = KLlossFn(BatchLogits, TeacherLogits, labels_batch)
            loss = 0.1*hardLoss +  0.9*softLoss
...
def main(params):
    ...
    KLlossFn = KLLossfnWithT(T=4, reduction='mean')
    alpha = params.alpha
    BestValAcc = -1
    BestValAccEpoch = -1
