...
def trainOneEpoch(model, Teacher, trainloader, optimizer, alpha, beta, temperature):
    model.train()
    Teacher.eval()
    Nsamples = 0
    Loss = 0
    ...
    with tqdm(total=len(trainloader)) as t:
        for Nbatch, (train_batch, labels_batch) in enumerate(trainloader):
            ...
            loss = dkd_loss(BatchLogits, TeacherLogits, labels_batch, alpha, beta, temperature)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
def main(params):
    ...
    for epoch in range(params.num_epochs):
        TrainRes = trainOneEpoch(model, Teacher, trainloader, optimizer, alpha, beta, temperature)
